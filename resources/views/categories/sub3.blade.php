        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-header">
                    <a href="{{url('/category')}}">ទំព័រដើម</a> > <a href="{{url('/sub1/'.$parent->id)}}">{{$parent->name}}</a> > <a href="{{url('/sub2/'.$sub2->id)}}">{{$sub1->name}}</a>
                    > <strong>{{$sub2->name}}</strong> &nbsp;
                    {{--<a href="#" id="btnAddRegistration" class="btn-link" onclick="addItem()">បន្ថែម</a>--}}
                </div>
                <div class="card-block">
                    {{csrf_field()}}
                    <input type="hidden" id="parent_id" value="{{$sub2->id}}">
