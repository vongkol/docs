        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-header">
                    <a href="{{url('/category')}}">ទំព័រដើម</a> > <a href="{{url('/sub1/'.$parent->id)}}">{{$parent->name}}</a> > <strong>{{$sub1->name}}</strong> &nbsp;
                    <a href="#" id="btnAddRegistration" class="btn-link" onclick="addItem()">បន្ថែម</a>
                </div>
                <div class="card-block">
                    {{csrf_field()}}
                    <input type="hidden" id="parent_id" value="{{$sub1->id}}">
                    <ul class="list" id="list">
                        @foreach($subs as $cat)
                            <li id="{{$cat->id}}"><a href="{{url('/sub3/'.$cat->id)}}">{{$cat->name}}</a>&nbsp;&nbsp;
...
@section("js")
    <script>
        ...
        function cancelEdit(obj)
        {
            var li = $(obj).parent();
            var id = $(li).attr("id");
            var str = "<a href='" + burl + "/sub3/" + id + "'>" + xname + "</a>&nbsp;&nbsp;";
        ...
        function saveEdit(obj) {
            ...
            $.ajax({
                ...
                success: function (sms) {
                    if (sms > 0) {
                        var str = "<a href='" + burl + "/sub3/" + cat.id + "'>" + cat.name + "</a>&nbsp;&nbsp;";
                        str += "<a href='#' class='text-success' onclick='edit(this,event)'><i class='fa fa-pencil'></i></a>&nbsp;";
                        str += "<a href='#' class='text-danger' onclick='rm(this,event)'><i class='fa fa-remove'></i></a>";
                        $(li).html(str);
                    }
        ...
        function save(obj) {
            ...
            $.ajax({
                ...
                success: function (sms) {
                    if (sms > 0) {
                        $(li).attr("id", sms);
                        var str = "<a href='" + burl + "/sub3/" + sms + "'>" + cat.name + "</a>&nbsp;&nbsp;";
                        str += "<a href='#' class='text-success' onclick='edit(this,event)'><i class='fa fa-pencil'></i></a>&nbsp;";
                        str += "<a href='#' class='text-danger' onclick='rm(this,event)'><i class='fa fa-remove'></i></a>";
                        $(li).html(str);
                    }
