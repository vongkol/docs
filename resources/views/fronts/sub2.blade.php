...
@section("content")
    ...
    <div class="row">
        <div class="col-sm-12">
            <ul class="folder-list">
                @foreach($sub2s as $sub)
                    <?php
                    $count = DB::table("documents")->where("category_id", $sub->id)->where("type", "sub2")->count();
                    $sub2 = DB::table("sub3")->where("parent_id", $sub->id)->where("active",1)->count();
                    ?>
                    <li>
                        <a href="{{url('/front/sub3/'.$sub->id)}}">
    ...
    <div class="row">
        <div class="col-sm-12">
            <br>
            <p class="text-center">
                <a href="{{url('/front/sub1/'.$root->id)}}" class="btn btn-success"><img src="{{asset('img/back.png')}}" alt=""> &nbsp;ត្រលប់ក្រោយ</a>
            </p>
        </div>
    </div>
@endsection